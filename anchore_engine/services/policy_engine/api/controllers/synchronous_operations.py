    ...
    try:
        ...
        for k, v in Gate.registry.items():
            ...
            for t in v.__triggers__:
                ...
                if t.__params__:
                    for param_name, validator in t.__params__.items():
                        tps = TriggerParamSpec()
                        tps.name = param_name
                        tps.description = ''
                        tps.validator = ''
                        tps.validator_description = ''

                        if hasattr(validator, '__validator_description__'):
                            tps.validator_description = validator.__validator_description__ if validator.__validator_description__ else ''

                        if hasattr(validator, 'validation_criteria'):
                            tps.validator = validator.validation_criteria() if validator.validation_criteria() else ''
